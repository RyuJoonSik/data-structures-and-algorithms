function 배열_합_구하기(배열: number[]): number {
  if (배열.length === 1) {
    return 배열[0];
  }
...
function 문자열_뒤집기(문자열: string): string {
  if (문자열.length === 1) {
    return 문자열[0];
  }
...
function X_세기(문자열: string): number {
  if (문자열.length === 0) {
    return 0;
  }
...
function 계단_경로_수_세기(계단_수: number): number {
  if (계단_수 < 0) {
    return 0;
...
function 에너그램_생성(문자열: string): string[] {
  if (문자열.length === 1) {
    return [문자열[0]];
  }
...
function 최대_수_구하기(배열: number[]): number {
  if (배열.length === 1) {
    return 배열[0];
  }
...
function 최대_수_구하기_개선(배열: number[]): number {
  if (배열.length === 1) {
    return 배열[0];
  }
