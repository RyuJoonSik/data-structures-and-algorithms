...
function 최적화_두_수의_합_구하기(배열: number[]) {
  let 해시_테이블: { [숫자: number]: boolean } = {};
...
function 동전_게임_승자_찾기(동전_개수: number, 현재_플레이어 = 'you'): string {
  if (동전_개수 <= 0) {
    return 현재_플레이어;
  }
  ...
  if (
    동전_게임_승자_찾기(동전_개수 - 1, 다음_플레이어) === 현재_플레이어 ||
...
function 최적화_동전_게임_승자_찾기(동전_개수: number) {
  if ((동전_개수 - 1) % 3 === 0) {
    return 'them';
  } else {
    return 'you';
...
function 합_교환하기(배열1: number[], 배열2: number[]) {
  const 해시_테이블: { [숫자: number]: number } = {};
  let 배열1_합 = 0,
    배열2_합 = 0;
...
function 최대_부분_합_구하기(배열: number[]) {
  let 현재_합 = 0;
  let 최대_합 = 0;
...
function 애너그램인지(단어_1: string, 단어_2: string) {
  const 단어_해시_테이블: { [문자: string]: number } = {};
  const 단어_2_해시_테이블: { [문자: string]: number } = {};
...
function 그룹_정렬하기(배열: string[]) {
  const 해시_테이블: { [문자: string]: number } = {};
  const 새_배열: string[] = [];
